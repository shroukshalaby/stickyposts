...
class user_manager_model extends MY_Model
{
    ...
    */
    ...
    public function login($email = "", $password = "", $remember = false)
    {
        ...
        $user = $this->model->get("id,status,ids,password", $this->tb_users, "email = '" . addslashes($email) . "'");


        if ($user == null || !(password_verify($password, $user->password))) {
            ms([
                "status" => "error",
                "message" => __("The account you entered does not match any account")
            ]);
            return false;
        }
    ...
    public function recovery_password($recovery_key, $password, $confirm_password)
    {
        ...
        $this->db->update($this->tb_users, ["password" => password_hash($password, PASSWORD_BCRYPT), "changed" => now()], ["id" => $user->id]);
    ...
    public function update_password($current_password, $password, $confirm_password)
    {
        ...
        $this->db->update($this->tb_users, ["password" => password_hash($password, PASSWORD_BCRYPT)], ["id" => $uid]);
