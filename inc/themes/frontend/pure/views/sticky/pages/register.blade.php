        ...
        <div class="col-md-6 col-12 mx-auto">
          <div class="box_shadow p-15">
            <div class="btn-group btn-group-toggle w-100" data-toggle="buttons">
              <label id="btnSticky"   class="btn active sticky-active-background" onclick="ChangeBg(this.id)"><input type="radio" name="options" id="option1" autocomplete="off" checked> {{__('stickyposts')}}</label>
              <label id="btnAuto"     class="btn" onclick="ChangeBg(this.id)"> <input type="radio" name="options" id="option2" autocomplete="off"> {{__('b_AutoActivity')}} </label>
              <label id="btnAnalyzer" class="btn" onclick="ChangeBg(this.id)"> <input type="radio" name="options" id="option3" autocomplete="off"> {{__('b_Competitors')}} </label>
              <label id="btnFamily"   class="btn" onclick="ChangeBg(this.id)"> <input type="radio" name="options" id="option4" autocomplete="off"> {{__('b_FamilyService')}} </label>
            </div>
            <form action="/pure/ajax_register" data-hide-overplay="false" data-redirect="https://stickyposts.net/payment" class="actionRegister form sticky-active-background">
                <div class="row">
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for="fullname"><span class="colo-r">*</span> {{__('reg-label-text6')}} </label>
                    ...
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for="lastname"><span
                                    class="colo-r">*</span> {{__('reg-label-text7')}}
                        </label>
                    ...
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for="email"> <span class="colo-r f-s-15">*</span> {{__('reg-label-text8')}}</label>
                        <input type="email" class="form-control" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" value="" name="email" required="">
                     </div>
                    </div>
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for="pwd"><span class="colo-r f-s-15">*</span> {{__('reg-label-text9')}} </label>
                    ...
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for=""><span
                                    class="colo-r">*</span> {{__('reg-label-text10')}}
                        </label>
                    ...
                    <div class="col-md-6 col-12">
                     <div class="form-group m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                        <label for="activeCode"> <span class="colo-r f-s-15">*</span> {{__('reg-label-text11')}}</label>
                    ...
                    <div class="col-md-12 col-12 text-start">
                      <div class="row">
                          <div class="col-12 mt-2">
                             <label class="{{  getCurrentLangIdiom() == 'ar' ? 'w-100 text-right'  : 'text-left' }}" for=""> {{__('reg-p-text12')}}</label>
                          </div>
                           <div class="col-md-6 col-12">
                            <div class="form-group {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }} mb-0">
                            <div class="custom-control custom-radio">
                                <input type="radio" class="custom-control-input" id="yesCheck" name="yesno" value="resources">
                                <label class="custom-control-label pure-checkbox" for="yesCheck"> {{__('reg-p-text13')}}   </label>
                            </div>
                        </div>   
                          </div>
                           <div class="col-md-6 col-12">
                            ...
                            <div class="pure-checkbox grey mb-1 w-100">
                                <input type="checkbox" id="md_checkbox_schedule" name="terms" class="filled-in chk-col-red enable_instagram_schedule" value="on" required="">
                                    <span class="checkbox-text-left mx-1">  {{__('reg-span-text14')}} 
                                    <a href="{{get_url('terms')}}" class="t-d-0 colo-b f-w-b"> {{__('reg-a-text15')}}</a>
                                    </span>
                    ...
                    <div class="col-12">
                        <p class="f-s-10 text-white m-0 {{  getCurrentLangIdiom() == 'ar' ? ' text-right'  : 'text-left' }}">
                            <span class="colo-r f-s-15">(*)</span> {{__('reg-p-text11')}}
                        </p>
                    </div>
                    ...
                    <div class="text-center col-12">
                        <button type="submit" class="btn btn-block mt-0 btn-loading d-none mx-auto col-sm-6 col-md-5 col-lg-5 col-xl-5 btn-register">
                                <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> {{__('reg-btn-text48')}}
                        </button>
                        <button type="submit" class="btn btn-block mt-0 btn-no-loading mx-auto col-sm-6 col-md-5 col-lg-5 col-xl-5 btn-register">
                               <i class="fas fa-sign-in-alt"></i> {{__('reg-btn-text48')}}
                        </button>
                    </div>
        ...
        <div class="col-md-6 d-none d-lg-block">
            <div class="card-container box_shadow" style="padding: 15px;" id="content">
                <div class="card-header text-center text-dark py-2" style="background:linear-gradient(90deg,#320b82,#9c33c3);">
                    <h3 style="margin: 0;color:#fff">{{__('stickyposts')}}</h3>
                </div> 
                <div class="row align-items-center">
                 ...
                 <div class="col-md-6 px-2 text-start text-black">
                    <div class="stack__card__content">
                        <h2 class="stack__card__title">{{__('reg-h2-text71')}}</h2>
                        <p>{{__('reg-p-text72')}}</p>
                    </div>   
                 </div>
                </div>
...
@endsection
@push('scripts')
    <script src="{{_e(get_theme_frontend_url('assets/js/core.js')) }}"></script>
    <script src="{{_e(get_theme_frontend_url('assets/js/pure.js')) }}"></script>
    <script>
